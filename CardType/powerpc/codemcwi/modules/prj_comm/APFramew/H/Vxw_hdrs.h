/*****************************************************************************
 * vxw_hdrs.h - includes the vxWorks definitions and function prototypes
 *              needed to compile applications programs in the vxWorks-to-
 *              pthreads environment.  NOTE that the actual original vxWorks
 *              header files introduce conflicts with some native Linux
 *              header files and consequently cannot be used in the pthreads
 *              environment.
 *  
...
**  types of the objects they reference as defined in the vxWorks-to-pthreads
...
**
**  The following three functions are unique to VxWorks-to-pthreads. 
...
extern STATUS    wdStart( WDOG_ID wdId, int delay, FUNCPTR funcptr, int parm );
 ...
 extern  pthread_t 	getLogpid();

 extern int exception_task( int dummy0, int dummy1, int dummy2, int dummy3,
