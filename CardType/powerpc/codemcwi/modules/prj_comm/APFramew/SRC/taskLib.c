 ...
 *             to implement a VxWorks task control API in a POSIX Threads
 *             environment.
 *  
...
*/
#if __cplusplus//modify by huangjl
extern "C" {
#endif
extern void selfRestart( vxwk2pthread_cb_t *restart_tcb );
...
**            tasks at once.
*/
vxwk2pthread_cb_t *task_list = (vxwk2pthread_cb_t *)NULL;
...
/*
**  vxworks_task_lock is a mutex used to make taskLock exclusive to one
**                    thread at a time.
*/
pthread_mutex_t vxworks_task_lock = PTHREAD_MUTEX_INITIALIZER;

/*
**  scheduler_locked contains the pthread ID of the thread which currently
**                   has the scheduler locked (or NULL if it is unlocked).
*/
static pthread_t scheduler_locked = (pthread_t)NULL;
    ...
    */
    if ( task_list != (vxwk2pthread_cb_t *)NULL )
    {
        for ( current_tcb = task_list;
              current_tcb != (vxwk2pthread_cb_t *)NULL;
...
/*add byhuangjl*/
    ...
    /*
    **  vxworks_task_lock ensures that only one VxWorks pthread at a time gets
    **  to run at max_priority (effectively locking out all other VxWorks
    ...
    */
    do {
        ...
        */
        pthread_mutex_lock( &vxworks_task_lock );
        ...
        else
        {
            ...
            printf( "\r\ntaskLock locking tid %ld my tid %ld",
                    scheduler_locked, my_pthrid );
#endif
            pthread_cond_wait( &taskLock_change, &vxworks_task_lock );
        }
        pthread_mutex_unlock( &vxworks_task_lock );
    ...
    /*
    **  task_list_lock prevents other VxWorks pthreads from modifying
    **  the VxWorks pthread task list while we're searching it and modifying
...
/*add byhuangjl*/
    struct sched_param sch_param;
    /*
    **  scheduler_locked ensures that only one VxWorks pthread at a time gets
    **  to run at max_priority (effectively locking out all other VxWorks
    ...
    */
    ...
    if ( scheduler_locked == pthread_self() )
    {
        ...
        if ( taskLock_level < 1L )
        {
            /*
            **  task_list_lock prevents other VxWorks pthreads from modifying
            **  the VxWorks pthread task list while we're searching it and
        ...
        sch_param.sched_priority = tcb->prv_priority.sched_priority;;
...
{
    vxwk2pthread_cb_t *nxt_entry;

    if ( list_head != (vxwk2pthread_cb_t **)NULL )
    {
        pthread_cleanup_push( (void(*)(void *))pthread_mutex_unlock,
                              (void *)&task_list_lock );
        pthread_mutex_lock( &task_list_lock );
        new_entry->nxt_susp = (vxwk2pthread_cb_t *)NULL;
        if ( *list_head != (vxwk2pthread_cb_t *)NULL )
        {
            for ( nxt_entry = *list_head; 
                  nxt_entry->nxt_susp != (vxwk2pthread_cb_t *)NULL;
   ...
   unlink_susp_tcb( vxwk2pthread_cb_t **list_head, vxwk2pthread_cb_t *entry )
{
    ...
    if ( list_head != (vxwk2pthread_cb_t **)NULL )
    {
        ...
        entry->nxt_susp = (vxwk2pthread_cb_t *)NULL;
...
{
    vxwk2pthread_cb_t *signalled_task;
    vxwk2pthread_cb_t *current_tcb;
    int result;
    ...
    */
    if ( task_list != (vxwk2pthread_cb_t *)NULL )
    {
        ...
        */
        new_taskid = 0;
        for ( current_tcb = task_list; 
              current_tcb->nxt_task != (vxwk2pthread_cb_t *)NULL;
              current_tcb = current_tcb->nxt_task )
        {
            /*
...
/*****************************************************************************
** translate_priority - translates a VxWorks priority into a pthreads priority
*****************************************************************************/
static int
   translate_priority( int vxworks_priority, int sched_policy, int *errp )
{
    ...
    */
    if ( (vxworks_priority > MIN_VXWK_PRIORITY) | 
         (vxworks_priority < MAX_VXWK_PRIORITY) )
        *errp = S_taskLib_ILLEGAL_PRIORITY;
 
    /*
    **  Translate the VxWorks priority into a pthreads priority.
    **  'Invert' the caller's priority so highest priority == highest number.
    */
    pthread_priority = MIN_VXWK_PRIORITY - vxworks_priority;
    ...
    */
    ...
    /*
    **  'Telescope' the VxWorks priority (0-255) into the smaller pthreads
    **  priority range (1-97) on a proportional basis.  NOTE that this may
    **  collapse some distinct VxWorks priority levels into the same priority.
    ...
    **  tasks are within fewer than 3 levels of one another.
    ...
    pthread_priority /= (MIN_VXWK_PRIORITY + 1);
    */
    ...
    **  NOTE that relative VxWorks priorities may not translate properly
    **  if the VxWorks priorities used span several multiples of max_priority.
...
*****************************************************************************/
static void
   tcb_delete( vxwk2pthread_cb_t *tcb )
{
    vxwk2pthread_cb_t *current_tcb;
...
*****************************************************************************/
void
   notify_task_delete( vxwk2pthread_cb_t *tcb )
{
    /*
    ...
    */
    ...
    printf( "\r\nnotify_task_delete - wait till pended tasks respond @ tcb %p",
            tcb );
#endif
    while ( tcb->first_susp != (vxwk2pthread_cb_t *)NULL )
...
{
    vxwk2pthread_cb_t *mytcb;

    mytcb = (vxwk2pthread_cb_t *)tcb;
    pthread_mutex_lock( &vxworks_task_lock );
...
/*****************************************************************************
**  task_wrapper is a pthread used to 'contain' a VxWorks task.
*****************************************************************************/
void *
    task_wrapper( void *arg )
{
    ...
    **  free its memory before beginning the VxWorks task
    */
    tcb = (vxwk2pthread_cb_t *)arg;
    ...
    */
    ...
    /*
    **  Call the VxWorks task.  Normally this is an endless loop and doesn't
    **  return here.
    */
#ifdef DIAG_PRINTFS 
   ...
   taskDelay( int interval )
{
    ...
    */
    usec = (unsigned long)(interval * VXWK_TICK * 1000);
   ...
   taskIdListGet( int list[], int maxIds )
{
    ...
    /*
    **  'Lock the VxWorks scheduler'
    */
    taskLock();

    if ( (task_list != (vxwk2pthread_cb_t *)NULL) && (list != (int *)NULL) )
    {
        /*
        **  One or more tasks already exist in the task list...
   ...
   taskIdSelf( void )
{
    ...
    if ( self_tcb != (vxwk2pthread_cb_t *)NULL )
...
/*****************************************************************************
** taskInit - initializes the requisite data structures to support VxWorks 
              ...
              int arg6, int arg7, int arg8, int arg9, int arg10 )
{
    ...
    if ( tcb != (vxwk2pthread_cb_t *)NULL )
    {
        ...
        */
        ...
        tcb->suspend_list = (vxwk2pthread_cb_t **)NULL;
        tcb->nxt_susp = (vxwk2pthread_cb_t *)NULL;
        tcb->nxt_task = (vxwk2pthread_cb_t *)NULL;
        ...
        */
        tcb->first_susp = (vxwk2pthread_cb_t *)NULL;
...
/*****************************************************************************
** taskActivate -  creates a pthread containing the specified VxWorks task
*****************************************************************************/
STATUS
    taskActivate( int tid )
{
    ...
    /*
    **  'Lock the VxWorks scheduler' to defer any context switch to a higher
...
//    printf( "\r\nin taskActivate\n");
    ...
    if ( tcb != (vxwk2pthread_cb_t *)NULL )
    {
        ...
        */
        if ( tcb->state == DEAD )
        {
            tcb->state = READY;
#ifdef DIAG_PRINTFS 
            printf( "\r\ntaskActivate task @ %p tcb @ %p:", tcb->entry_point, tcb );
#endif
    ...
    /*
    **  'Unlock the VxWorks scheduler' to enable a possible context switch
...
/*****************************************************************************
** taskSpawn -   initializes the requisite data structures to support VxWorks 
**               task behavior not directly supported by Posix threads and
**               creates a pthread to contain the specified VxWorks task.
*****************************************************************************/
int
               ...
               int arg6, int arg7, int arg8, int arg9, int arg10 )
{
    ...
    tcb = ts_malloc( sizeof( vxwk2pthread_cb_t ) );
    if ( tcb != (vxwk2pthread_cb_t *)NULL )
    {
        /*
...
/*****************************************************************************
** taskSuspend - suspends the specified VxWorks task
*****************************************************************************/
STATUS
    taskSuspend( int tid )
{
...
/*****************************************************************************
** taskResume - creates a pthread to contain the specified VxWorks task and
*****************************************************************************/
STATUS
    taskResume( int tid )
{
    ...
    if ( tcb != (vxwk2pthread_cb_t *)NULL )
    {
        ...
        */
        if ( (tid != 0) && (tcb != my_tcb()) )
        {
            pthread_attr_setschedparam( &(tcb->attr), &(tcb->prv_priority) );
			pthread_attr_getschedparam(&(tcb->attr),&sch_param);
            sch_param.sched_priority = new_priority;
        pthread_setschedparam( tcb->pthrid, sched_policy,
                          (struct sched_param *)(&sch_param));;
        } 
    } 
...
/*****************************************************************************
** taskName - returns the name of the specified VxWorks task
*****************************************************************************/
char *
    taskName( int tid )
{
...
/*****************************************************************************
** taskNametoId - identifies the named VxWorks task
*****************************************************************************/
int
        ...
        if ( task_deletable == FALSE )
        {
            ...
            else
            {
                ...
                */
                ...
                self_tcb->suspend_list = (vxwk2pthread_cb_t **)NULL;
