...
*****************************************************************************/
static void
    send_msg_to( vxwk_mqueue_t *queue, char *msg, uint msglen )
{
    uint i;
    char *element;
...
*****************************************************************************/
static void
    notify_if_delete_complete( vxwk_mqueue_t *queue )
{
    /*
    ...
    */
    if ( (queue->first_susp == (vxwk2pthread_cb_t *)NULL) &&
         (queue->first_write_susp == (vxwk2pthread_cb_t *)NULL) )
    {
        /*
...
*****************************************************************************/
static void
    fetch_msg_from( vxwk_mqueue_t *queue, char *msg )
{
    char *element;
    uint i;
    ...
    */
    if ( queue->first_write_susp != (vxwk2pthread_cb_t *)NULL )
    {
...
*****************************************************************************/
static q_msg_t *
    data_extent_for( vxwk_mqueue_t *queue )
{
    char *new_extent;
    char *last_msg;
    size_t alloc_size;
...
/*****************************************************************************
** msgQCreate - creates a VxWorks message queue
*****************************************************************************/
vxwk_mqueue_t *
    msgQCreate( int max_msgs, uint msglen,  int opt )
{
    ...
    */
    queue = (vxwk_mqueue_t *)ts_malloc( sizeof( vxwk_mqueue_t ) );
    if ( queue != (vxwk_mqueue_t *)NULL )
    {
        /*
        ...
        */
        if ( data_extent_for( queue ) != (q_msg_t *)NULL )
        {
            ...
            ** a message from queue
            */
            queue->first_susp = (vxwk2pthread_cb_t *)NULL;
...
*****************************************************************************/
static int
    waiting_on_q_space( vxwk_mqueue_t *queue, struct timespec *timeout,
                        int *retcode )
{
    int result;
    struct timeval now;
...
*****************************************************************************/
STATUS
   waitToSend( vxwk_mqueue_t *queue, char *msg, uint msglen, int wait, int pri )
{
...
/*****************************************************************************
** msgQSend - posts a message to the tail of a VxWorks queue and awakens the
**            first selected task pended on the queue.
*****************************************************************************/
STATUS
   msgQSend( vxwk_mqueue_t *queue, char *msg, uint msglen, int wait, int pri )
{
	printf("-----in msgQSend-----\n");
#ifdef DIAG_PRINTFS 
    vxwk2pthread_cb_t *our_tcb;
...
*****************************************************************************/
static void
   delete_mqueue( vxwk_mqueue_t *queue )
{
    /*
...
*****************************************************************************/
STATUS
   msgQDelete( vxwk_mqueue_t *queue )
{
    STATUS error;
    ...
    if ( queue_valid( queue ) )
    {
        ...
        */
             ...
             (queue->first_write_susp != (vxwk2pthread_cb_t *)NULL) )
        {
            ...
            */
            while ( (queue->first_susp != (vxwk2pthread_cb_t *)NULL) &&
                    (queue->first_write_susp != (vxwk2pthread_cb_t *)NULL) )
            {
...
*****************************************************************************/
static int
    waiting_on_q_msg( vxwk_mqueue_t *queue, struct timespec *timeout,
                       int *retcode )
{
    int result;
    struct timeval now;
...
**               specified VxWorks queue.
*****************************************************************************/
STATUS
   msgQReceive( vxwk_mqueue_t *queue, char *msgbuf, unsigned int buflen, int max_wait )
{
    vxwk2pthread_cb_t *our_tcb;
    struct timeval now;
    struct timespec timeout;
            ...
            else
            {
                ...
                */
                ...
                else
                {
                    ...
                    */
                    fetch_msg_from( queue, (char *)msgbuf );
#ifdef DIAG_PRINTFS 
...
*****************************************************************************/
int
   msgQNumMsgs( vxwk_mqueue_t *queue )
{
    int num_msgs;
