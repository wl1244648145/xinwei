 ...
 *            to implement a VxWorks semaphore API in a POSIX Threads
 *            environment.
 *  
...
/*****************************************************************************
**  Control block for VxWorks semaphore
**
...
**  the attributes of a VxWorks semaphore.
**
*****************************************************************************/
typedef struct vxwk_sema4
{
        /*
...
*****************************************************************************/
static int
   sema4_valid( vxwk_sema4_t *sema4 )
{
    
    vxwk_sema4_t *current_smcb;
    int found_sema4;
    ...
    */
    ...
    if ( sema4_list != (vxwk_sema4_t *)NULL )
    {
        /*
        ...
        */
        for ( current_smcb = sema4_list; 
              current_smcb != (vxwk_sema4_t *)NULL;
...
*****************************************************************************/
static void
   link_smcb( vxwk_sema4_t *new_sema4 )
{
    vxwk_sema4_t *current_smcb;
    ...
    */
    ...
    new_sema4->nxt_sema4 = (vxwk_sema4_t *)NULL;
    if ( sema4_list != (vxwk_sema4_t *)NULL )
    {
        /*
        **  One or more semaphores already exist in the semaphore list...
        ...
        */
        for ( current_smcb = sema4_list; 
              current_smcb->nxt_sema4 != (vxwk_sema4_t *)NULL;
              current_smcb = current_smcb->nxt_sema4 );
...
*****************************************************************************/
static vxwk_sema4_t *
   unlink_smcb( vxwk_sema4_t *smid )
{
    vxwk_sema4_t *current_smcb;
    vxwk_sema4_t *selected_smcb;

    selected_smcb =  (vxwk_sema4_t *)NULL;
...
/*****************************************************************************
** new_sema4 - creates a new VxWorks semaphore using pthreads resources
*****************************************************************************/
vxwk_sema4_t *
    new_sema4( int count )
{
    vxwk_sema4_t *semaphore;

    /*
    **  First allocate memory for the semaphore control block
    */
    semaphore = (vxwk_sema4_t *)ts_malloc( sizeof( vxwk_sema4_t ) );
    if ( semaphore != (vxwk_sema4_t *)NULL )
    {
        /*
        ...
        ** First task control block in list of tasks waiting on semaphore
        */
        semaphore->first_susp = (vxwk2pthread_cb_t *)NULL;
    }
...
/*****************************************************************************
** semBCreate - creates a VxWorks binary semaphore
*****************************************************************************/
vxwk_sema4_t *
...
/*****************************************************************************
** semCCreate - creates a VxWorks counting semaphore
*****************************************************************************/
vxwk_sema4_t *
...
/*****************************************************************************
** semMCreate - creates a VxWorks mutual exclusion semaphore
*****************************************************************************/
vxwk_sema4_t *
    semMCreate( int opt )
{
...
*****************************************************************************/
STATUS
   semDelete( vxwk_sema4_t *semaphore )
{
#ifdef DIAG_PRINTFS 
    ...
    if ( sema4_valid( semaphore ) )
    {
        ...
        */
        ...
        if ( semaphore->first_susp != (vxwk2pthread_cb_t *)NULL )
        {
            ...
            */
            while ( semaphore->first_susp != (vxwk2pthread_cb_t *)NULL )
...
*****************************************************************************/
STATUS
   semFlush( vxwk_sema4_t *semaphore )
{
#ifdef DIAG_PRINTFS 
    vxwk2pthread_cb_t *our_tcb;
#endif
...
/*****************************************************************************
** semGive - releases a VxWorks semaphore token and awakens the first
**           selected task waiting on the semaphore.
*****************************************************************************/
STATUS
   semGive( vxwk_sema4_t *semaphore )
{
    vxwk2pthread_cb_t *our_tcb;
    STATUS error;
             ...
             (semaphore->current_owner != (vxwk2pthread_cb_t *)NULL) )
        {
            ...
            */
            if ( owners_priority < my_priority )
            {
                ...
                sch_param.sched_priority = my_priority;
