 ...
 *           to implement a VxWorks watchdog timer API in a POSIX Threads
 *           environment.
 *  
...
/*****************************************************************************
**  Control block for VxWorks watchdog timer
**
...
**  In the VxWorks-to-pthreads environment, these timers execute from the
**  context of the system exception task rather tha the timer interrupt.
*****************************************************************************/
typedef struct vxwk_wdog
{
        /*
...
*****************************************************************************/
static int
   wdog_valid( vxwk_wdog_t *wdId )
{
    
    vxwk_wdog_t *current_wdog;
    int found_wdog;
...
*****************************************************************************/
static void
   link_wdog( vxwk_wdog_t *new_wdog )
{
    vxwk_wdog_t *current_wdog;
    ...
    */
    ...
    new_wdog->nxt_wdog = (vxwk_wdog_t *)NULL;
    if ( wdog_list != (vxwk_wdog_t *)NULL )
    {
        /*
        **  One or more watchdogs already exist in the watchdog list...
        ...
        */
        for ( current_wdog = wdog_list; 
              current_wdog->nxt_wdog != (vxwk_wdog_t *)NULL;
              current_wdog = current_wdog->nxt_wdog );
...
*****************************************************************************/
static vxwk_wdog_t *
   unlink_wdog( vxwk_wdog_t *wdId )
{
    vxwk_wdog_t *current_wdog;
    vxwk_wdog_t *selected_wdog;

    selected_wdog =  (vxwk_wdog_t *)NULL;
...
*****************************************************************************/
static vxwk_wdog_t *
   process_tick_for( vxwk_wdog_t *wdId )
{
    vxwk_wdog_t *nxt_wdog;
        ...
        if ( wdId->ticks_remaining > 0 )
        {
            ...
            */
                 ...
                 (wdId->timeout_func != (void (*)( int ))NULL) )
            {
                ...
                **  preempted at this point by other VxWorks tasks, and
   ...
   process_timer_list( void )
{
    ...
    if ( wdog_list != (vxwk_wdog_t *)NULL )
    {
        ...
        */
        for ( nxt_wdog = wdog_list; nxt_wdog != (vxwk_wdog_t *)NULL; )
        {
#ifdef DIAG_PRINTFS 
...
vxwk_wdog_t *
   wdCreate( void )
{
    vxwk_wdog_t *new_wdog;
        
     /*
     **  Allocate memory for the watchdog timer control block.
     */
    new_wdog = (vxwk_wdog_t *)ts_malloc( sizeof( vxwk_wdog_t ) );
    if ( new_wdog != (vxwk_wdog_t *)NULL )
    {
         /*
...
*****************************************************************************/
STATUS
   wdStart( vxwk_wdog_t *wdId, int delay, void (*funcptr)( int ), int parm )
{
    STATUS error;
...
*****************************************************************************/
static vxwk_wdog_t *restart_wdog;
...
/*****************************************************************************
** selfRestart - restarts the specified (existing) VxWorks task after said
**               task has terminated itself.
*****************************************************************************/
void
   selfRestart( vxwk2pthread_cb_t *restart_tcb )
{
    /*
    ...
    */
    ...
    if ( restart_wdog != (vxwk_wdog_t *)NULL )
    {
        /*
