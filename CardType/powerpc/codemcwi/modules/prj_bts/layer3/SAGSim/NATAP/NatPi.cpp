...
int NatPiSession::sendNatPiPkt(UINT8 *pDataToSend, UINT16 nLenToSend)
{
	...
	if(m_fdSocket!=-1 && nLenToSend>0 && pDataToSend!=NULL)
	{
		...
		{
			//发送给SAG
			int totalToSend=nLenToSend;
			char* bufToSend = (char*)pDataToSend;
			do 
			{
				sent = send(m_fdSocket, bufToSend, nLenToSend, 0);
				if(sent<0)
				{
					LOG1(LOG_DEBUG2, LOGNO(VOICE, EC_L3VOICE_NORMAL), 
...
//AuthResult
	...
	NATAPComputeAuthResult( natRegRspMsg.AHHead.SPI, 
							(UINT8*) &natRegRspMsg, 
							(UINT8*)&natRegRspMsg.NatRegRspData - (UINT8*)&natRegRspMsg,
...
UINT8 NatPiSession::packNATAPMsg4AppPkt(UINT8* pDataApp)
{
	int i;
	UINT32 *pUINT32;
	//1)NATAP AHhead
	//2)LinkID
	//3)0--64字节，4字节整数倍
	//由于ComMessage头部空间和TCPheader限制，本实现为0--28字节
	UINT8 nRandFillUINT32 = rand() & 0x0F;	//0--7
	UINT16 nNatApPktLen = sizeof(AH_T)+
						  sizeof(NATAP_LinkIDT)+
						  sizeof(UINT32)*nRandFillUINT32;
	...
	pNat_AppPkt->AHHead.nLen = htons(nNatApPktLen-1);//去掉Protocol和SPI
	pNat_AppPkt->AHHead.msgType = APP_PACKET;
	//LinkID
	pNat_AppPkt->linkID = htons(getLinkID());
	//random filled UINT32s
	pUINT32 = (UINT32*)pNat_AppPkt->buf;
	for(i=0;i<nRandFillUINT32;i++)
	{
		*pUINT32++ = rand();
	}
	return nNatApPktLen;
...
NATAP_PKT_TYPE NatPiSession::authenticateNATAPMsg(UINT8* pMsg, UINT16 len)
{
	...
	switch(pktType)
	{
		...
		case REG_MSG:
			...
			pInput = (UINT8*)pMsg;
			inputLen = (UINT8*)&pRegPkt->NatRegData - pMsg;
		...
		case REG_RSP_MSG:
			...
			pInput = (UINT8*)pMsg;
			inputLen = (UINT8*)&pRegRspPkt->NatRegRspData - pMsg;
			digestLen = NATAPComputeAuthResult(spi, pInput, inputLen, Digest);
			if(0==digestLen) 
			{
				LOG1(LOG_DEBUG2, LOGNO(VOICE, EC_L3VOICE_NORMAL), 
...
NatPiSession g_VCR_NatAPc(NAT_APP_TCP, M_TID_VCR, "VCR");
//global obj for voice DMUX udp NATAP
NatPiSession g_VDR_NatAPc(NAT_APP_UDP, M_TID_VDR, "VDR");
