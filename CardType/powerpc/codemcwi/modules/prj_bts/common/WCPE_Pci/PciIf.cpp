 ...
 *---------------------------------------------------------------------------*/
void  mvI2o0CircularQueueEnable(I2O_CIRCULAR_QUEUE_SIZE cirQueSize, UINT32 queueBaseAddr);
...
bool CTaskPciIf::Initialize()
{
    ...
    MsgQPciIf = msgQCreate(OUT_BOUND_Q_SIZE, sizeof(PCI_IF_Q_MSG),MSG_Q_FIFO);
    ...
    for (int i=0; i<BUF_POOL_NUM; i++)
    {
        bufCount += PciTxBufNum[i];
        bufSize += PciTxBufNum[i] * PciTxBufSize[i];
    }
    TotalBufferCount = bufCount;
    if (bufSize > PEER_TX_BUF_SIZE)
    {
    ...
    UINT32 bufAddr = PEER_TX_BUF_BASE_REMAP_ADDR; 
    for (int poolIndex=0; poolIndex<BUF_POOL_NUM; poolIndex++)
    {
        ...
        for (int bufIndex=0; bufIndex<PciTxBufNum[poolIndex]; bufIndex++)
        {
            ...
            nodePtr->BufferAddress = bufAddr;
            nodePtr->BufferStatus = FREE;

            #ifdef CHECK_BUFFER_FREE_FLAG
            *(UINT32*)bufAddr = PCI_BUF_FREE_FLAG;
            #endif

            bufAddr += PciTxBufSize[poolIndex];
            lstAdd(&FreePeerTxBufList[poolIndex], &nodePtr->lstHdr);
            nodePtr++;
        }

                // allocate memory for Rx buffer status table
        RxBufStatusTable[poolIndex] = (T_RxBufStatus*)malloc(PciTxBufNum[poolIndex] 
    ...
    memset((char *)CircularQMem, 0xff, PCI_CIRCULAR_BUF_SIZE);
    mvI2o0CircularQueueEnable(CircularQSize, CircularQMem);
    OnflightTxMsgPostPendingBuf = (PCI_MSG*)malloc(TotalBufferCount * sizeof(PCI_MSG));
    PendingTxMsgPostPendingBuf = (PCI_MSG*)malloc(TotalBufferCount * sizeof(PCI_MSG));
    OnflightTxMsgPostPendingCount = 0;
    PendingTxMsgPostPendingCount = 0;

    RxMsgFreePendingBuf = (PCI_MSG*)malloc(TotalBufferCount * sizeof(PCI_MSG));
...
bool CTaskPciIf::PostMessage(CComMessage* msg, SINT32 timeOut, bool isUrgent)
{
	...
	#ifdef M_TGT_L2
	if (M_TID_EB == msg->GetDstTid() && gWAN_CPE_EID == msg->GetEID()) 
    {
        msg->SetDstTid(M_TID_WANIF);
        return CComEntity::PostEntityMessage(msg);
	}
    #endif
    
    if ( (UINT32)msg->GetBufferPtr() >= PEER_TX_BUF_BASE_REMAP_ADDR)
    { // need to make a copy if the packet is looped back to the peer
      // due to DMA address decoding issue, we can not DMA data from 
      // 0x0F000000 to the other end, have to make a copy

        CComMessage * relayMsg = msg;
        msg = new (this, relayMsg->GetDataLength())CComMessage;
        if ( NULL == msg )
        {   
            return false;
        }
        msg->SetDstTid ( relayMsg->GetDstTid());
        msg->SetSrcTid ( relayMsg->GetSrcTid());
        msg->SetMessageId( relayMsg->GetMessageId());
        msg->SetDataLength( relayMsg->GetDataLength());
        msg->SetEID ( relayMsg->GetEID());
        msg->SetUID ( relayMsg->GetUID());
        msg->SetFlag (0xffffffff);
        memcpy ( msg->GetDataPtr(),  relayMsg->GetDataPtr(),  msg->GetDataLength());
        //relayMsg->Destroy();   /*by xiaoweifang. tEB destroy ComMessage with NULL creator*/// need to destory the original ComMessage
    }
    ...
    if (isUrgent)
    {
        rc = msgQSend(MsgQPciIf, (char*)(&postMsg), sizeof(PCI_IF_Q_MSG),NO_WAIT, MSG_PRI_URGENT);
    }
    else
    {
        rc = msgQSend(MsgQPciIf, (char*)(&postMsg), sizeof(PCI_IF_Q_MSG),NO_WAIT, MSG_PRI_NORMAL);
    }
    if ( OK != rc)
    {
        msg->Release();
        LOG(LOG_CRITICAL, LOG_ERR_PCI_QUEUE_OVERFLOW, "Post ComMessage to PCI interface message q failed");
        OutboundMsgPostFailCount++;
        return false;
    }
    else
    {
...
void CTaskPciIf::CheckTxTokenSanity()
{
    ...
    {
        if (tickGet() - TxPostMsgTokenLostTick > 50)
        {
            LOG(LOG_CRITICAL, LOG_ERR_PCI_POST_BUSY_RX_BUF,"Tx Post msg Token Reclaimed by timeout");
...
void CTaskPciIf::MainLoop()
{
    ...
    *(volatile UINT32*)(PCI0_MEM0_BASE | MV64360_I2O_IB_MSG0_CPU0) = 
    ...
    const UINT32 eventMaskBothQ =   EV_MSG_Q  | EV_EVENT_Q;
    const UINT32 eventMaskEventQ = EV_EVENT_Q;
    ...
    for(;;)
    {
        ...
        {
            STATUS rc = msgQEvStart(MsgQPciIf, EV_MSG_Q, EVENTS_SEND_IF_FREE );
...
void CTaskPciIf::OutBoundMsgProc()
{
    ...
    while ( sizeof(PCI_IF_Q_MSG) == msgQReceive(MsgQPciIf, (char*)&msg, sizeof(PCI_IF_Q_MSG), NO_WAIT))
    {
        firstDmaDesc = NULL;
        lastDmaDesc = NULL;
        ...
        {  // a ComMessage
            ...
            firstDmaDesc=TxDmaChannel->CreateDesc((UINT8*)(node->BufferAddress + L2_L3_PKT_LEADING_SPACE), 
                                                          (UINT8*)header,
                                                          msgLen
                                                          );
        ...
        {   // IP packets
            ...
            UINT32 destAddr = node->BufferAddress;
            ...
            else
            {
                ...
                continue;  // forward to prepare next packet, no need to operate on DMA list
            }
        }
...
UINT16 quetail = InboundMsgCirQue.QueueTail;
    ...
    while (InboundMsgCirQue.QueueTail != InboundMsgCirQue.QueueHead)
    {
        ...
        else if (pciMsg.Msg.MessageLength >=  4096)
        {
        ...
        else if ( PCI_RX_BUF_OCCUPIED == RxBufStatusTable[poolIndex][bufIndex].BufferStatus )
        {
            ...
            LOG3(LOG_CRITICAL, LOG_ERR_PCI_POST_BUSY_RX_BUF, 
                 ...
                 poolIndex, bufIndex, PeerTxBufNode[poolIndex][bufIndex].BufferAddress);
        ...
        else if (pciMsg.Msg.MessageFormat == TYPE_COMMSG)
        {
            ...
            UINT8* bufPtr = (UINT8*)PeerTxBufNode[poolIndex][bufIndex].BufferAddress;
...
bool CTaskPciIf::DeallocateComMessage(CComMessage* pComMsg)
{
    ...
    {  // to free a message using PCI Tx buffers
        ...
        UINT16 poolIndex = pciMsg.Msg.PoolIndex;
        UINT16 bufIndex = pciMsg.Msg.BufferIndex;
        if (poolIndex >= BUF_POOL_NUM || bufIndex>=PciTxBufNum[poolIndex])
        {
        ...
        {
            UINT32 *bufAddr = (UINT32*)PeerTxBufNode[poolIndex][bufIndex].BufferAddress;
            if (pComMsg->GetBufferPtr() != (void *)PeerTxBufNode[poolIndex][bufIndex].BufferAddress)
            {
                LOG2(LOG_CRITICAL, LOG_ERR_PCI_APP_FREE_INVALID_BUF,
                     "App free with buffer pointer changed -- should be %x, is %x",
                     (int)PeerTxBufNode[poolIndex][bufIndex].BufferAddress,
    ...
    while (outBoundFreeQueHead != outBoundFreeQueTail)
    {
        ...
        if (outBoundFreeQueTail & 0x4)
        {
            ...
            if (   pciMsg.Msg.MUST_BE_ZERO 
                ...
                || pciMsg.Msg.BufferIndex>= PciTxBufNum[pciMsg.Msg.PoolIndex] )
            {
            ...
            else
            {
                ...
                UINT32 *peerBufAddr = (UINT32*)(node->BufferAddress - PEER_TX_BUF_BASE_REMAP_ADDR + PEER_TX_BUF_BASE_ADDR);
        ...
        case L2_PCI_IF_READY_FOR_BOOT:
            {
                ...
                for (int poolIndex=0; poolIndex<BUF_POOL_NUM; poolIndex++)
                {
                    ...
                    for (int bufIndex=0; bufIndex<PciTxBufNum[poolIndex]; bufIndex++)
                    {
                        ...
                        // add Rx buffer status on 4/17/2006, by Shi,Yushu
                        RxBufStatusTable[poolIndex][bufIndex].BufferStatus = PCI_RX_BUF_EMPTY;
                        RxBufStatusTable[poolIndex][bufIndex].pComMsg = NULL;
                        RxBufStatusTable[poolIndex][bufIndex].PostRound = 0;

                        bufAddr = (UINT32*)PeerTxBufNode[poolIndex][bufIndex].BufferAddress;
...
STATUS CTaskPciIf::PciEndSend( M_BLK_ID pMblk)
{
    ...
    postMsg.mBlk = pMblk;
    STATUS rc = msgQSend(MsgQPciIf, (char*)(&postMsg), sizeof(PCI_IF_Q_MSG),NO_WAIT, MSG_PRI_NORMAL);
    if ( OK != rc)
    {
...
void CTaskPciIf::ShowStatus(int verbose)
{
    ...
    for (int poolIndex=0; poolIndex<BUF_POOL_NUM; poolIndex++)
        printf("0x%08x|", PeerTxBufNode[poolIndex][0].BufferAddress);
    printf("\n");

    printf("Buffer Count             |");
    for (int poolIndex=0; poolIndex<BUF_POOL_NUM; poolIndex++)
        printf("%10d|", PciTxBufNum[poolIndex]);
    printf("\n");
    ...
    printf("OutboundMsgPostFailCount = %u\n", OutboundMsgPostFailCount);
...
/*******************************************************************************
* mvI2o0CircularQueueEnable - Initialize the I2O queue messaging mechanism.
*
* DESCRIPTION:
...
*
*******************************************************************************/
void  mvI2o0CircularQueueEnable(I2O_CIRCULAR_QUEUE_SIZE cirQueSize,
...
UINT32 CTaskPciIf::GetBufAddr(UINT16 poolIndex, UINT16 bufIndex)
{
    if (poolIndex < BUF_POOL_NUM && bufIndex<PciTxBufNum[poolIndex])
    {
        return PeerTxBufNode[poolIndex][bufIndex].BufferAddress;
    }
    else
    {
...
extern "C" void StartL3If(void)
{
   ...
   #ifdef M_TGT_L2
   WanCpeIF *wanIf = WanCpeIF::GetInstance();
   wanIf->Begin();
   #endif
}
...
extern "C" int displayPciMsg(UINT32 flag)
{
    ...
    msg.Val = flag;
    UINT32 addr = CTaskPciIf::GetInstance()->GetBufAddr(msg.Msg.PoolIndex, msg.Msg.BufferIndex);
