...
int i2cDoOpAD7417
	...
	if ( I2C_CYCLE_READ(unit,(unsigned char *)pI2cCmdPacket->memoryAddress+0,1) )
	    {
	...
	if ( pI2cCmdPacket->nBlocks == 2 )
	    {
	 //   logMsg("he3\n",0,1,2,3,4,5);
	    if ( I2C_CYCLE_READ_New(unit,(unsigned char *)pI2cCmdPacket->memoryAddress+1,0,1 ))
		{
	//	logMsg("he4\n",0,1,2,3,4,5);
		statusVariable = I2C_ERROR_CYCLE_READ;
		goto errorEnd ;
...
int i2cDoOp
    ...
    switch(pI2cCmdPacket->deviceType)
		...
		return ERROR ;
            break;
         case I2C_RTC_TYPE_PCF8563:
             if (i2cDoOpPCF8563(unit, deviceAddress, pI2cCmdPacket) != OK )
		return ERROR ;
            break;
        default :
            return ERROR ;
