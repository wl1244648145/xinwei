...
int i2cCycleMpc85xxRead
   ...
   readData =  i2cIoctl(I2C_IOCTL_RD,
            ...
            0, 0);
    #endif
//  logMsg("*pReadDataBuf1:%x\n",(unsigned char)readData ,0,1,2,3,4);
...
//    logMsg("*pReadDataBuf:%x\n",*pReadDataBuf ,0,1,2,3,4);
#if 0
    if (!ack)
        {
        /* Don't send master ack. */
...
/** DEBUG FUNCTIONS **/
void i2cReadTest(unsigned int index)
{
      unsigned char a;
          if ( i2cRead (0,
                  0x50,
                  ...
                  &a) == OK )
              	{
              	       printf("i2cReadTest:%x\n",a);
              	}
}
void i2cWriteTest(unsigned int index ,unsigned char writevalue )
{
 //  char readvalue;
    ...
    if ( i2cRead (0,
                  ...
                  &readvalue) == OK )
        {
            if(writevalue!=readvalue)
            	{
            	   printf("value,readvalue:%x,%x,%d\n",writevalue,readvalue,index);
            	   return ;
            	}
        }
   #endif
...
* ERRNO: N/A
*/
int i2cReadByte(unsigned char *ptr,unsigned int len)
{
...
int i2cWriteByte(unsigned char *ptr,unsigned int len)
{

   // int index = 0;
 //   char verifyData[REAL_SPD_SIZE];
    char readvalue,writevalue;
   int unit  =0;
   unsigned int deviceAddress = 0x50;
   int i;
   #if 0
    ...
    )
{
      if ( i2cRead (unit,
                  deviceAddress,
                  I2C_RTC_TYPE_PCF8563,
                  reg,
                  size,
                 (char*)data) == ERROR )
        {
        logMsg("%d :i2crtcDateTimeGet Driver returned error\n",__LINE__,0,0,0,0,0);
        return(ERROR);
        }
    ...
    )
{
    ...
    if(i2cWrite (
           unit,
        deviceAddress,	/* Device's I2C bus address */
    I2C_RTC_TYPE_PCF8563,
    reg,	/* starting block to write */
   size,	/* number of blocks to write */
     data			/* pointer to buffer of send data */
    )==ERROR)
    	{
    	        logMsg("%d : i2crtcTimeSet Driver returned error\n",__LINE__,0,0,0,0,0);
       	 return(ERROR);
    	}
}
