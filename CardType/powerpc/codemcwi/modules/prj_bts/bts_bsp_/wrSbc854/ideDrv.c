...
--------------------
01r,07sep04,jyo  Fixing SPR 100119: Removing references to RT11.
01q,02dec03,rcs  fixed warnings for base6
...
retryPread:
    ...
    sysInWordString (IDE_DATA, buffer, 256);
...
retryRW:
	...
	for (ix = 0; ix < nSecs; ix++)
	    {
            ideWait (STAT_DRQ);
	    sysOutWordString (IDE_DATA, (void *)pBuf, nWords);
            semStatus = semTake (&ideSyncSem, sysClkRateGet() * ideSemSec);
    	    if ((ideStatus & STAT_ERR) || (semStatus == ERROR))
	        goto errorRW;
	    pBuf += nWords;
	...
	for (ix = 0; ix < nSecs; ix++)
	    {
	        ...
	        goto errorRW;
            ideWait (STAT_DRQ);
	    sysInWordString (IDE_DATA, (void *)pBuf, nWords);
	    pBuf += nWords;
	    }
	}
